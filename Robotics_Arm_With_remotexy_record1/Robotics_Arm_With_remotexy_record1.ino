...
{
  ist[0] = map(SensVal[0], -100, 100, 600, 2400);//  drehen
  ist[1] = map(SensVal[1], -100, 100, 550, 2400);// Schulter
...
void play_servo()
{
    ...
    if (steps < stepsMax) // sure we not reach the end from a move
    {
      ...
      else if(steps == 100) time1 = del-1;    // cannot explain here is not del*1
